               ...
               @foreach($filters as $filter)
                ...
                @admin
                   @foreach($adminFilters as $adminFilter)
                       <option value="{{ $adminFilter }}">{{ $adminFilter }}</option>
                   @endforeach
                @endadmin
            </select>
        </div>
        <div class="w-full md:w-2/3 relative">
            <input wire:model.live="search" type="search" placeholder="Find an idea" class="w-full rounded-xl bg-white border-none placeholder-gray-900 px-4 py-2 pl-8">
